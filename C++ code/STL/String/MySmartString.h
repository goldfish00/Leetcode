...
//  lab3
//
//  Created by Junyi Huang on 2/3/24.
//  Copyright © 2024 Junyi Huang. All rights reserved.
//

#ifndef MySmartString_h
#define MySmartString_h
...
private:
    ...
    void grow_allocation(int amount) {
        ...
        allocation = 2 * amount;
        std::unique_ptr<char[]> doubled_alloc = std::make_unique<char[]>((size_t)allocation);
        strcpy(doubled_alloc.get(), cstr.get());
        
        cstr = std::move(doubled_alloc);
    }

    // allocates a C-string, constructed on the range from begin to end of C-string str.
    std::unique_ptr<char[]> allocate_string(const char* str, size_t begin, size_t len) const {
    std::unique_ptr<char[]> alloc_str = std::make_unique<char[]>(len + 1);
        strncpy(alloc_str.get(), str + begin, len);
        alloc_str.get()[len] = a_null_byte;
        return std::move(alloc_str);
    }
...
// Default initialization is to contain an empty string with no allocation.
// If a non-empty C-string is supplied, this String gets minimum allocation.
String::String(const char* cstr_):sz(0), allocation(0){
    sz = strlen(cstr_);
    cstr = allocate_string(cstr_, 0, sz);
    allocation = sz + 1;
...
// The copy constructor initializes this String with the original's data,
// and gets minimum allocation.
String::String(const String& original): sz(original.sz), allocation(original.sz + 1) {
    cstr = std::make_unique<char[]>(allocation);
    strcpy(cstr.get(), original.cstr.get());
...
// Move constructor - take original's data, and set the original String
// member variables to the empty state (do not initialize "this" String and swap).
String::String(String&& original) noexcept: sz(original.sz), allocation(original.allocation) {
    cstr = std::move(original.cstr);
}
...
char& String::operator[] (size_t i) {
   if (i > sz) throw String_exception("subscript index out of range.");
...
char& String::front() {
    if (!cstr) {
        throw String_exception("try to dereference a null string");
    }
...
String String::substr(size_t pos, size_t len) const {
    String sub;
    sub.allocation = min(sz - pos, len);
    sub.sz = sub.allocation - 1;
    sub.cstr = allocate_string(cstr.get(), pos, sub.allocation);
    return sub;
}
...
String& String::erase(size_t pos, size_t len) {
    if (pos >= sz) {
        throw String_exception("invalid starting pos for erase");
    }
    len = min(sz - pos, len);

    for (size_t copyFrom = pos + len, copyTo = pos; copyFrom <= sz; ++copyFrom, ++copyTo) {
        cstr.get()[copyTo] = cstr.get()[copyFrom];
    }
...
String& String::insert(size_t pos, const String& str) {
    if (pos > sz) throw String_exception("invalid insert pos");

    if (str.sz == 0) {
        return *this;
    }

    size_t new_sz = sz + str.sz;
    check_allocation(new_sz + 1);

    size_t copyTo = sz + str.sz;
    for (size_t copyFrom = sz;; --copyFrom, --copyTo) {
        cstr[copyTo] = cstr[copyFrom];
    ...
    // copy str: copy back, since str might == this
    for (size_t copyFrom = str.sz - 1;; --copyFrom, --copyTo) {
        //std::cout << copyFrom << " " << copyTo << "\n";

        cstr[copyTo] = str[copyFrom];
        if (copyFrom == 0) {
            break;
        }
    }
...
String& String::replace(size_t pos, size_t len, const String& str) {
    size_t erase_pos = str.sz + pos;
    // must insert first then erase
    // otherwirse, bugs will occur if it is the same string object
    insert(pos, str);
    erase(erase_pos, len);
...
String& String::operator += (const char* rhs) {
    size_t rhs_sz = strlen(rhs);
    check_allocation(sz + rhs_sz + 1);

    for (size_t copyTo = sz, copyFrom = 0; copyFrom < rhs_sz; ++copyTo, ++copyFrom) {
    cstr[copyTo] = rhs[copyFrom];
    }
