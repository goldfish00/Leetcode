...
//  lab3
//
//  Created by Junyi Huang on 2/2/24.
...
// The copy constructor initializes this String with the original's data,
// and gets minimum allocation.
String::String(const String& original): cstr(allocate_string(original.cstr, 0, original.sz)), sz(original.sz), allocation(static_cast<int>(original.sz) + 1) {
    ++number;
    total_allocation += allocation;
}
...
// Move constructor - take original's data, and set the original String
// member variables to the empty state (do not initialize "this" String and swap).
String::String(String&& original) noexcept: cstr(original.cstr), sz(original.sz), allocation(original.allocation) {
...
String String::substr(size_t pos, size_t len) const {
    ...
    return allocate_string(cstr, pos, min(len, sz - 1 - pos));
}
...
String& String::erase(size_t pos, size_t len) {
    len = min(sz - pos, len);
    size_t copyPos = pos + len;
    while (copyPos < sz) {
        cstr[pos++] = cstr[copyPos++];
    }
...
String& String::insert(size_t pos, const String& str) {
    ...
    check_allocation(static_cast<int>(new_sz + 1 /* the null byte */));
    std::cout << cstr << "\n";
    if (pos != sz) {
        for (size_t front = sz - 1, back = new_sz - 1; ; --front, --back) {
...
//    std::cout << cstr << "\n";
    ...
    // insert with the same string object
    if (!(this == &str && pos == 0)) {
        char* strPtr = (this == &str) ? cstr : str.cstr;
        for (size_t front = pos + str.sz - 1, back = str.sz - 1; ; --front, --back) {
            cstr[front] = strPtr[back];
//            std::cout << pos << " " << str.sz << " " << new_sz << " " << front << " " << back << "\n";
            if (front == pos) {
                break;
            }
        }
...
String& String::replace(size_t pos, size_t len, const String& str) {
    if (pos >= sz) throw String_exception("Substring bounds invalid");
    if ((pos == 0 && this == &str) || len == 0) {
        return *this;
    }
    
    size_t original_sz = str.sz;
    insert(pos, str);
    // std::cout << cstr << "\n";
    erase(pos + original_sz, len);
    // std::cout << cstr << "\n";
...
size_t String::find(const String& str, size_t pos) const {
    ...
    for (int res = pos; res <= sz - str.sz; ++res) {
        bool found = true;
        
        for (int i = 0; i < str.sz; ++i) {
            if (cstr[res + i] != str.cstr[i]) {
                found = false;
                break;
            }
        }
